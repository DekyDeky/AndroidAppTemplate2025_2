...
class DashboardFragment : Fragment() {
    ...
    private fun salvarItem() {
        ...
        if (tarefa.isEmpty() || descricaoTarefa.isEmpty() || dataInicio.isEmpty() || dataFim.isEmpty() || imageUri == null) {
        ...
        if (itemId != null) {
            databaseReference.child(auth.uid.toString()).child(itemId).setValue(item)
                .addOnSuccessListener {
                    Toast.makeText(context, "Tarefa cadastrado com sucesso!", Toast.LENGTH_SHORT)
                        .show()
                    requireActivity().supportFragmentManager.popBackStack()
                }.addOnFailureListener {
                    Toast.makeText(context, "Falha ao cadastrar o Tarefa", Toast.LENGTH_SHORT).show()
                }
        } else {
